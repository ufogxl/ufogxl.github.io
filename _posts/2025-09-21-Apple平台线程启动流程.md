...
## libdispatch三种机制的典型应用场景

#### CPU密集型任务（Worker机制）
```c
dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
    // 图像处理、数据解析、算法计算等CPU密集型工作
...
flowchart TB
    subgraph US ["🔵 用户空间"]
        A[dispatch_async<br/>异步任务提交]
        
        I[_start_wqthread<br/>工作线程汇编入口]
        J[__pthread_wqthread<br/>pthread工作线程入口<br/>_pthread_wqthread处理]
        K{线程类型判断}
        L[__libdispatch_workloopfunction<br/>workloop线程处理函数]
        M[__libdispatch_keventfunction<br/>kevent事件处理函数]
        N[__libdispatch_workerfunction<br/>普通工作线程处理函数]
        O[🎯 执行用户block<br/>调用用户代码]
    ...
    subgraph KS ["🔴 内核空间"]
        B[_dispatch_kq_fill_workloop_event<br/>填充workloop事件<br/>kevent_qos系统调用]
        C[workq_kern_threadreq_initiate<br/>内核工作队列线程请求初始化]
        D[workq_add_new_idle_thread<br/>添加新的空闲线程<br/>thread_create_workq_waiting]
        E[workq_park_and_unlock<br/>线程等待停放并解锁]
        F[workq_thread_wakeup<br/>唤醒工作线程]
        G[thread_setrun<br/>设置线程运行状态<br/>调度器运行<br/>thread_go → call_continuation]
        H[workq_unpark_continue<br/>取消线程停放继续运行<br/>workq_setup_and_run<br/>workq_setup_thread]
        Q[workq_set_register_state<br/>设置寄存器状态<br/>thread_set_wq_state64<br/>PC=_start_wqthread]
        R[machine_switch_context<br/>机器上下文切换<br/>arm64_load_context]
